...
def changeMagOrPhase(mag, phase):
    #mag[:,319:322] = 0 # von x=319 bis x=322 setzt es auf kompletter y-Hoehe die werte auf 0
    mag[0:200,:] = 0
    mag[280:480,:] = 0
    mag[:,0:200] = 0
    mag[:,440:640] = 0
...
def main():
    ...
    result, mag, phase = changeMagOrPhase(mag,phase)
    # show the resulting image
    title_result = 'Frequencies image after Change'
    cv2.namedWindow(title_result, cv2.WINDOW_NORMAL) # Note that window parameters have no effect on MacOS
    cv2.resizeWindow(title_result,window_width,window_height)
    cv2.imshow(title_result,result)
