...
# load the DNN model with cv2.dnn.readNet()
net = cv2.dnn.readNet(model='models/frozen_inference_graph_ssd.pb',
                      config='models/ssd_mobilenet_v2_coco_2018_03_29.pbtxt', framework='TensorFlow')
...
conf_thresh = 0.2  # TODO: play with this value
max_object_size = 0.9  # TODO: play with this value
mean = (104, 117, 123)
...
while cap.isOpened():
    ...
    if ret:
        ...
        blob = cv2.dnn.blobFromImage(image=image, size=(
            int(image_width/4), int(image_width/4)), mean=mean, swapRB=True)
        # start time to calculate FPS
        start = time.time()
        # set the blob as input
        net.setInput(blob)
        # the inference: compute the output by running a forward pass
        output = net.forward()
