...
while True:
    # if there are four reference points, then compute the transform and apply the transformation
    if not(computationDone) and (len(refPtSrc) == 4 and len(refPtDst) == 4):
        T_perspective = cv2.getPerspectiveTransform(np.float32(refPtSrc), np.float32(refPtDst), solveMethod=cv2.DECOMP_SVD)
